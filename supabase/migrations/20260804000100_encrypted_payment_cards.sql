-- Replaces the VGS card-reveal integration with card storage encrypted at
-- rest (AES-256-GCM, key held in PAYMENT_CARD_ENCRYPTION_KEY) and decrypted
-- server-side only for an authorized advisor. This is a deliberate reversal
-- of payment_vault_foundation.sql's original "raw PANs must never enter this
-- database" design: VGS's reveal capability is disabled by default and
-- requires VGS Support to enable it per vault, which made both the
-- client-side widget and the server-side Reveal API unusable. This stays a
-- sandbox/test-card-only feature; a real PCI compliance review is required
-- before this pattern ever touches a genuine customer card.
...
alter table public.payment_credentials
  drop constraint payment_credentials_provider_check,
  add constraint payment_credentials_provider_check check (provider in ('vgs', 'scout_encrypted'));

create or replace function public.add_encrypted_payment_credential(
  portal_token text,
